...
export default function App() {
  ...
  useEffect(() => {
    ...
    const checkExpirations = async () => {
      ...
      for (const asgn of assignments) {
        ...
        // If expired and we haven't notified them yet
        if (now > expiryDate && !asgn.expiryNotified) {
          ...
          // Notify Admin
          sendEmail(ADMIN_EMAIL, `Service Alert: Expired Node (${asgn.clientEmail})`, 
            `User ${asgn.clientEmail} has an expired port (${asgn.port}).`);

          // Mark notified in DB to prevent duplicate emails
  ...
  const adminAssignTunnel = async (reqId, email, data, type) => {
    ...
    await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'assignments'), { 
      ...
      expiryNotified: false // Required for auto-email watcher
    });
          ...
          <div className="grid lg:grid-cols-3 gap-12">
            <div className="lg:col-span-2 space-y-10">
              ...
              {myReqs.filter(r => r.type === 'new' || r.type === 'trial' || r.type === 'renewal').map((req) => {
                ...
                // --- FULL MIKROTIK SCRIPT GENERATION ---
...
/ip firewall filter add action=accept chain=input comment="SwifftNet Top" place-before=0 src-address=192.168.89.0/24
