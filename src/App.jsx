...
const INTERGRAM_ID = "5631296198"; // <--- REPLACE THIS WITH YOUR TELEGRAM ID FROM @IntergramBot
...
export default function App() {
  ...
  /**
   * --- INTERGRAM CHAT INITIALIZATION ---
   */
  useEffect(() => {
    // Intergram configuration
    window.intergramId = INTERGRAM_ID;
    window.intergramCustomizations = {
      title: 'SwifftNet Support',
      introMessage: 'Hello! How can we help you today?',
      autoResponse: 'Connecting you to support...Pleasee do not leave',
      mainColor: '#2563eb', // SwifftNet Blue
      alwaysShow: false
    };

    // Inject the script
    const script = document.createElement('script');
    script.id = 'intergram-script';
    script.src = 'https://www.intergram.xyz/js/widget.js';
    script.async = true;
    document.body.appendChild(script);

    return () => {
      const el = document.getElementById('intergram-script');
        ...
        <div className="max-w-6xl mx-auto space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <header className="flex flex-col md:flex-row justify-between items-center bg-slate-900/50 p-8 rounded-[40px] border border-slate-800 gap-6 shadow-xl">
            ...
            <div className="flex gap-4">
               {/* Support Button */}
               <button 
                 onClick={() => window.intergram && window.intergram.open()}
