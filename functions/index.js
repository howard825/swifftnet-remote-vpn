...
/exports.updateVpnStatus = functions.https.onRequest(async (req, res) => {
  ...
  if (!vpnUser) return res.status(400).send("Missing vpnUser");
  ...
  try {
    ...
    // We check for the username exactly as sent, AND lowercase just in case
    const snapshot = await asgnRef.where("user", "in", [vpnUser, vpnUser.toLowerCase(), vpnUser.toUpperCase()]).get();

    if (snapshot.empty) {
      console.log(`404: User [${vpnUser}] not found in Firestore assignments.`);
      return res.status(404).send(`User ${vpnUser} not found`);
    }
